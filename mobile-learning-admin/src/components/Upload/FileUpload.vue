  ...
  >
    <div v-if="fileUrl" class="file-preview">
      <div v-if="isVideo" class="video-preview">
        <video :src="fileUrl" controls style="width: 100%; max-height: 200px" />
      </div>
    ...
    <div v-else class="upload-placeholder">
      <UploadOutlined />
      <div style="margin-top: 8px">点击上传{{ type === 'video' ? '视频' : 'PDF' }}文件</div>
...
const handleUpload = async ({ file, onSuccess, onError }) => {
  ...
  try {
    ...
    if (data.code === 200) {
      ...
      fileName.value = data.data.filename || file.name
